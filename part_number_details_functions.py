...
def before_merging(dfs):
    ...
    print("Number of DataFrames for merging:", len(dfs))
...
def extracting_tables_in_pages_using_pdf_plumber(pdf_path, page_numbers):
  with pdfplumber.open(pdf_path) as pdf:
    tables = []
    for page_number in page_numbers:
      page = pdf.pages[page_number - 1]  # Adjust for 0-based indexing
      table_findings = page.extract_tables()
      if table_findings:
        tables.extend(table_findings)
    return tables
