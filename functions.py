...
def header_intro():
    ...
    with col1:
        st.markdown("""
            ...
            <p style='font-size: 16px;'>version 1.2 - Last update 9/12/2024</p>
...
def extracting_pin_tables(file_path, part_number, number_of_pins, package_type, package_code):
    ...
    table_starting_page_number, table_start_string, table_stop_string, table_ending_page_number = extracting_pin_tables_functions.find_table_starting_and_stopping_based_on_pin_string(file_path, pin_configuration_pages, pin_string, package_string, part_number)    
    st.text(f" Table Starting Section : {table_start_string}, Table Stopping Section : {table_stop_string}, Starting Page Number : {table_starting_page_number}, Ending Page Number : {table_ending_page_number} " )
    ...
    #st.text(f"List of Page Numbers : {pin_table_pages}")
    ...
    noise_calculation_combo_dict = extracting_pin_tables_functions.calculate_differences(reduced_combo_dict, table_as_text)
    st.text(f"Mapping After noise filter Algo : {noise_calculation_combo_dict}")


    min_key = extracting_pin_tables_functions.find_best_match(noise_calculation_combo_dict)
    st.text(f"Min Key found : {min_key}")
...
def extracting_pin_tables_2(file_path, part_number, number_of_pins, package_type, package_code):
    #pin_string = f"{number_of_pins}-{package_type}"
    start_keyword = "symbol pin information"
    end_keyword = "symbol parameters"
    pin_configuration_pages  = part_number_details_functions.find_pages_between_keywords(file_path, start_keyword, end_keyword)
    ...
    return noise_calculation_combo_dict,dfs
...
'''def finding_closest_match_for_user(noise_calculation_combo_dict,dfs):
    min_key = extracting_pin_tables_functions.find_best_match(noise_calculation_combo_dict)
    st.text(f"Min Key found : {min_key}")
    final_pin_tables_to_be_merged, number= extracting_pin_tables_functions.get_dataframes_from_tuple(dfs, min_key)
    #st.text(f" Number of Selected Dataframes : {number}")
...
def finding_closest_match_for_user(noise_calculation_combo_dict, dfs, update_used_keys=True):
    min_key = find_next_best_match(noise_calculation_combo_dict)
    if not min_key:
        return None  # Stop if no matches available
    ...
    # Track used keys
    if update_used_keys:
        st.session_state["used_keys"].append(min_key)

    return merged_df



# Function to find the next best match
def find_next_best_match(noise_calculation_combo_dict):
    # Filter out keys already used
    remaining_keys = {k: v for k, v in noise_calculation_combo_dict.items() if k not in st.session_state["used_keys"]}
    
    # If no more keys are available
    if not remaining_keys:
        st.warning("No more matches available.")
        return None
    
    # Find the best match among remaining keys
    return extracting_pin_tables_functions.find_best_match(remaining_keys)


def handle_generate_next_match_button(noise_calculation_combo_dict, dfs):

    #user_feedback = "Incorrect Table? :material/thumb_down:"
    #selected = st.feedback("thumbs")

    # Check if the selected feedback matches the thumbs-down feedback
    #if selected == ":material/thumb_down:":
        next_pin_table = finding_closest_match_for_user(noise_calculation_combo_dict, dfs, update_used_keys=True)
        if next_pin_table is not None:
            print(f"You have used new reiteration function to generate pin extraction data")
            pass
            #st.subheader("Next Extracted Pin Table")
            #st.dataframe(next_pin_table)
        else:
            st.warning("No further matches available.")
